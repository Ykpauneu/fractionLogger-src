...
local scriptVersion = "v0.3.0-beta.pre"
          ...
          if doesFileExist(json) then
            ...
            if f then
              ...
              if updateversion ~= thisScript().version then
                ...
                update = false
                print(thisScript().version..': Обновление не требуется.')
              end
            end
          else
            print(thisScript().version..': Не могу проверить обновление. Смиритесь или проверьте самостоятельно на '..url)
            update = false
          end
        end
      end
